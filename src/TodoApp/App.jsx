...
// useDispatch와 slice에서 생성한 액션을 활용해 action creator를 생성.
const actions = todosSlice.actions

function useTodoActions() {
  const dispatch = useDispatch()
  const toggleTodo = useCallback(
    (id) => dispatch(actions.toggleTodo(id)),
    [dispatch]
  );
  const deleteTodo = useCallback(
    (id) => dispatch(actions.deleteTodo(id)),
    [dispatch]
  );
  const addTodo = useCallback(
    (title) => dispatch(actions.addTodo(title)),
    [dispatch]
  );
...
function TodosPage() {
  // redux의 action creator를 사용하도록 메서드를 추가.
...
function TodoFilter() {
  ...
  // context가 아닌 redux store에서 데이터를 얻어옴.
